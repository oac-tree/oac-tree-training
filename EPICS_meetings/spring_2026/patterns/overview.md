...
The second procedure use implicit sequences, meaning it will try to achieve the final goal (room entered) and will only take steps to achieve partial goals of those are needed. For example, when the person is already in the room, the procedure will not try to make sure that the door is open or unlocked.

`03_decision_tree.xml` and `04_improving_safety.xml` demonstrate two alternatives of switching between two different execution paths, based on a condition to be true or false. Their differences are mostly cosmetic, but users may prefer one or the other, depending on the use case. The second approach may be more readable when adapting an existing procedure to include a failure path.

`05_ppa_1.xml`, `06_ppa_2.xml` and `07_ppa_3.xml` demonstrate the Postcondition-Precondition-Action (PPA) pattern to achieve the same goal as the first two procedures. They start from a single-level PPA and then use backchaining to gradually include more intermediate steps. The final logic (in `07_ppa_3.xml`) is equivalent to the one in `02_implicit_sequences.xml`, in that it tries to achieve partial goals only when required. The main difference is that the PPA pattern leads to more nested trees. The advantage of the backchaining approach is that it provides the designer of the procedure a very structured way of composing the instruction tree from basic components.
